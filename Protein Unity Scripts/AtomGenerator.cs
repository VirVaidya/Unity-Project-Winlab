...
public class AtomGenerator : MonoBehaviour
{
    ...
    public AtomGenerator(GameObject protein, ReferenceFileWriter list, GameObject waterprefab)
    {
    ...
    public void GenerateAtom(string line)
    {
        ...
        { // Checks and adds each new element to set so duplicate meshes are not created.
    ...
    public void makemeshes()
    {
        ...
        for (int i = 0; i < childCount; i++)
        {
            ...
            CleanUpOldMeshes(child.gameObject);
        }
    }
    ...
    public static void CleanUpOldMeshes(GameObject parent)
    {
        foreach (Transform child in parent.transform)
        {
            Destroy(child.gameObject); // Destroy the old mesh objects
        }
    }
