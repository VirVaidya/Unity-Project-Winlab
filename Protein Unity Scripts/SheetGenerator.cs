...
public class SheetGenerator : MonoBehaviour
{
    ...
    private static int numPoints = 30; // How many points are placed to create an overall curve
    ...
    public void makemeshes()
    {
        ...
        for (int i = 0; i < childCount; i++)
        {
            ...
            AtomGenerator.CleanUpOldMeshes(child.gameObject);
        }
    }
    ...
    List<string[]> GetAtomsInRange(int start, int end, string chainId1, string chainId2, AtomGenerator atoms)
    {
        ...
        //Debug.Log($"Searching for {chainId1} {start} to {chainId2} {end}...");
        while ((int.Parse(allatoms[i][3]) >= start && int.Parse(allatoms[i][3]) < end) || allatoms[i][2].Trim() != chainId2) // Locates all atoms in between the start point and the end point of a certain chainID
        {
